...
const RSVPSection = ({
  ...
  void loading; // used for future loading state UI
  ...
  const filteredGroups = useMemo(() => {
    ...
    if (q.length < minSearchChars) return [] as Group[];
    return groups.filter((g) => {
      if (g.groupName.toLowerCase().includes(q)) return true;
      return g.members.some((m) => m.name.toLowerCase().includes(q));
    });
  }, [groups, searchQuery, minSearchChars]);

  const resultsOpen = searchQuery.trim().length >= minSearchChars && filteredGroups.length > 0;
  const needsMoreChars = searchQuery.trim().length > 0 && searchQuery.trim().length < minSearchChars;
            ...
            <div className="mb-8">
              ...
              {needsMoreChars && (
                <p className="mt-2 text-sm text-muted-foreground">
                  Enter at least {minSearchChars} letters to see your name.
                </p>
              )}
            </div>
            ...
            {selectedGroup && selectedGroupRows && (
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-xl font-semibold">Who will be attending?</h3>
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => {
